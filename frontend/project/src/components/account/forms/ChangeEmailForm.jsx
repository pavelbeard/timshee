...
const ChangeEmailForm = ({ onClose }) => {
  ...
  const [generateTokenMut, { error: genTokenErr, isError }] =
    useGenerateTokenMutation();
  const [msg, setMsg] = useState(null);
  const [error, setError] = useState(null);

  const formContainer = clsx("w-10/12", "md:w-3/5", "lg:w-2/5");

  const handleSubmit = async (e) => {
    e.preventDefault();
    generateTokenMut({ email })
      .unwrap()
  ...
  if (msg) {
    return (
      <div
        className="bg-white flex flex-col items-center justify-center p-6"
    ...
    return (
      ...
      >
        ...
        <form
          ...
          className={clsx("flex flex-col", formContainer)}
        >
          <CustomInput
            htmlFor="email"
            type="email"
