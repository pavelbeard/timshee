      ...
      <InfoContainer>
        ...
        <section className="h-full min-h-52 overflow-y-auto">
          {!lastOrder ? (
            ...
            <div data-last-order="">
              <div className={clsx(orderFields, "roboto-medium")}>
                {lastOrder?.order_number}
              </div>
              {lastOrder?.status === "completed" ? (
                <div className={orderFields}>
                  <span className="roboto-light">
                    {t("account:deliveredAt")}
                  </span>
                  <span className="roboto-light">
                    {new Date(deliveredAt).toDateString()}
                  </span>
                </div>
                ...
                <div>
                  <div className={orderFields}>
                    ...
                    <span className="roboto-light">
                      {t(`account.orders:${toCamelCase(lastOrder?.status)}`)}
                    </span>
                  </div>
                  <div className={orderFields}>
                    <span className="roboto-light">
                      {t("account:refundedAt")}
                    </span>
                    <span className="roboto-light">
                      {new Date(lastOrder?.updated_at).toDateString()}
                    </span>
                  </div>
                </div>
              ) : (
