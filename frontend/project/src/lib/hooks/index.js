...
import {
  useSignInMutation,
  useSignOutMutation,
} from "../../redux/features/api/authApiSlice";
...
import { ArrayAtPolyfil } from "../stuff";
...
export const useSignOut = () => {
  const dispatch = useDispatch();
  const { t } = useTranslation();
  const [signOut, { isLoading: isSignOutLoading, error: signOutError }] =
    useSignOutMutation();
  const signout = async () => {
    try {
      await signOut();
...
export const useSearchParameters = () => {
  ...
  const get = (key) => {
    ...
    return ArrayAtPolyfil(all, 0) || null;
  };
...
export const useResetFilters = () => {
  ...
  return {
    sizes: () => {
      setSearch((prev) =>
        [...prev].filter((v) => !search.getAll("sizes").includes(v[1])),
      );
      dispatch(resetSizes());
    },
    colors: () => {
      setSearch((prev) =>
        [...prev].filter((v) => !search.getAll("colors").includes(v[1])),
      );
      dispatch(resetColors());
    },
    types: () => {
      setSearch((prev) =>
        [...prev].filter((v) => !search.getAll("types").includes(v[1])),
      );
      dispatch(resetTypes());
    },
    collections: () => {
      setSearch((prev) =>
        [...prev].filter((v) => !search.getAll("collections").includes(v[1])),
      );
      dispatch(resetCollections());
    },
    categories: () => {
      setSearch((prev) =>
        [...prev].filter((v) => !search.getAll("categories").includes(v[1])),
      );
      dispatch(resetCategories());
    },
    orderBy: () => {
      setSearch((prev) =>
        [...prev].filter((v) => !search.getAll("o").includes(v[1])),
      );
...
export const useCategories = () => {
  ...
  return categories.map((category) => ({
    ...
    types: types
      ...
      .map(removeCategoryAndCheckedFromType),
  }));
};
