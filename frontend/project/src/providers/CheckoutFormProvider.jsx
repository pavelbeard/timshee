...
} from "../redux/features/api/orderApiSlice";
import { useCreatePaymentMutation } from "../redux/features/api/paymentApiSlice";
import {
  selectCurrentToken,
  selectCurrentUser,
} from "../redux/features/store/authSlice";
...
export const CheckoutFormProvider = ({ children }) => {
  ...
  const isDone = [countries, provinces, phoneCodes].every(Boolean);
  ...
  const stageName = {
    ...
    1: {
      title: t("orders.checkout:toInformation"),
      link: `/checkout/${orderId}/address-info`,
    },
    2: {
      title: t("orders.checkout:toShipping"),
      link: `/checkout/${orderId}/shipping-info`,
    },
  ...
  const setFormData = (e) => {
    ...
    setCheckoutData({
      ...formData,
      [formName]: { ...formData[formName], [name]: value },
    });
  };
  ...
  const setNewProvince = (e) => {
    ...
    setCheckoutData({
      ...formData,
      [formName]: { ...formData[formName], province: newProvince },
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (page === 1) {
      ...
      updateOrder({ orderId, data: shippingAddress })
        .unwrap()
        .then((res) => {
      ...
      const shippingMethod = shippingMethods.find(
        (sm) => sm.id === formData.shipping_method,
      );
      updateOrder({ orderId, data: shippingMethod })
        .unwrap()
        .then((res) => {
