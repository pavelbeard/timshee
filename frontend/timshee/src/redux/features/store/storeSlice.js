...
const storeSlice = createSlice({
    ...
    reducers: {
        setItemsObject: (state, action) => {
            ...
            const { totalSizes, totalColors, totalTypes, totalCollections, totalCategories } = action.payload;
            state.itemsObject = { items, pagesCount, totalItemsCount };
            state.sizes = mergeCheckboxData(
                totalSizes, current(state.sizes), 'size__value', 'total_sizes', 'value'
            );
            state.colors = mergeCheckboxData(
                totalColors, current(state.colors), 'color__name', 'total_colors', 'name'
            );
            state.types = mergeCheckboxData(
                totalTypes, current(state.types), 'item__type__name', 'total_types', 'name'
            );
            state.collections = mergeCheckboxData(
                totalCollections, current(state.collections), 'item__collection__name', 'total_collections', 'name'
            );
            state.categories = mergeCheckboxData(
                totalCategories, current(state.categories), 'item__type__category__name', 'total_categories', 'name'
            );
        },
        ...
        toggleCheckboxes(state, action) {
            ...
            const update = (arr) => arr.map((item) => item[key] === id ? { ...item, checked } : item);
            switch (category) {
                case 'sizes':
                    const updatedSizes = update(current(state.sizes))
                    state.sizes = updatedSizes;
                    state.filters = modifyFilters(current(state.filters), updatedSizes, checked, 'value');
                    state.selectedLength.sizes = state.sizes.filter(s => s.checked).length;
                    break;
                case 'colors':
                    const updatedColors = update(current(state.colors))
                    state.colors = updatedColors;
                    state.filters = modifyFilters(current(state.filters), updatedColors, checked, 'name');
                    state.selectedLength.colors = state.colors.filter(i => i.checked).length
                    break;
                case 'types':
                    const updatedTypes = update(current(state.types))
                    state.types = updatedTypes;
                    state.filters = modifyFilters(current(state.filters), updatedTypes, checked, 'name');
                    state.selectedLength.types = state.types.filter(i => i.checked).length
                    break;
                case 'collections':
                    const updatedCollections = update(current(state.collections))
                    state.collections = updatedCollections;
                    state.filters = modifyFilters(current(state.filters), updatedCollections, checked, 'name');
                    state.selectedLength.collections = state.collections.filter(i => i.checked).length
                    break;
                case 'categories':
                    const updatedCategories = update(current(state.categories))
                    state.categories = updatedCategories;
                    state.filters = modifyFilters(current(state.filters), updatedCategories, checked, 'name');
                    state.selectedLength.categories = state.categories.filter(i => i.checked).length
                    break;
                default:
                    break;
            }
        ...
        setOrderBy(state, action) {
            const updatedOrderBy = state.orderBy.map(o => o.value === action.payload
                ? { ...o, selected: true }
                : { ...o, selected: false });
            state.orderBy = updatedOrderBy;
            const foundValue = updatedOrderBy.find(o => o.value === action.payload)?.name;
            // if there is a value from orderBy in arr
            const isThere = updatedOrderBy.find(o => state.filters.find(f => f === o.name))?.name;
        ...
        resetSizes(state) {
            ...
            state.filters = state.filters.filter(s => state.sizes.includes(size => size.name === s));
            state.selectedLength.sizes = 0;
        },
        resetColors(state) {
            state.colors = state.colors.map(c => ({ ...c, checked: false }));
            state.filters = state.filters.filter(c => state.colors.includes(color => color.name === c));
            state.selectedLength.colors = 0;
        },
        resetTypes(state) {
            state.types = state.types.map(t => ({ ...t, checked: false }));
            state.filters = state.filters.filter(t => state.types.includes(type => type.name === t));
            state.selectedLength.types = 0;
        },
        resetCollections(state) {
            state.collections = state.collections.map(c => ({ ...c, checked: false }));
            state.filters = state.filters.filter(c => state.collections.includes(collection => collection.name === c));
            state.selectedLength.collections = 0;
        },
        resetCategories(state) {
            state.categories = state.categories.map(c => ({ ...c, checked: false }));
            state.filters = state.filters.filter(c => state.categories.includes(category => category.name === c));
            state.selectedLength.categories = 0;
        },
        resetOrderBy(state) {
            state.orderBy.map(s => s.value === "" ? { ...s, selected: true } : { ...s, selected: false } );
            state.filters = state.filters.filter(o => state.orderBy.includes(orderBy => orderBy.name === o));
        ...
        uncheckAll(state) {
            ...
            state.collections = state.collections.map(c => ({ ...c, checked: false }));
            state.categories = state.categories.map(c => ({ ...c, checked: false }));
            state.orderBy = [...state.orderBy].map(s => s.value === ""
                ? { ...s, selected: true }
                : { ...s, selected: false }
            );
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addMatcher(apiSlice.endpoints.getItem.matchFulfilled, (state, action) => {
                state.sizesWithSelection = action?.payload?.sizes?.map((s, i) => i === 0
                    ? ({ ...s, selected: true })
                    : ({ ...s, selected: false })
                );
                state.colorsWithSelection = action?.payload?.colors?.map((c, i) => i === 0
                    ? ({ ...c, selected: true })
                    : ({ ...c, selected: false })
                );
            ...
            .addMatcher(apiSlice.endpoints.getWishlistByUser.matchFulfilled, (state, action) => {
...
export const selectTotalItemsCount = state => state.store?.itemsObject?.totalItemsCount || 0;
...
export const selectSizesWithSelection = state => state.store.sizesWithSelection;
export const selectColorsWithSelection = state => state.store.colorsWithSelection;
export const selectStocks = state => state.store.stocks;
export const selectWishlist = state => state.store.wishlist;
export const selectWishlistLength = state => state.store.wishlist?.length || 0;