...
export const cartSlice = createSlice({
    ...
    extraReducers: builder => {
        builder
            .addMatcher(apiSlice.endpoints.getCartItems.matchFulfilled, (state, action) => {
                state.cartItems = action?.payload?.cart_items || [];
                state.totalQuantity = action?.payload?.total_items || 0;
                state.totalPrice = action?.payload?.total || 0.00
            })
            .addMatcher(apiSlice.endpoints.addItemsToOrder.matchFulfilled, (state, action) => {
