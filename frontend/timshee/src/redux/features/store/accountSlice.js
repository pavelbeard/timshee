...
const accountSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addMatcher(
        apiSlice.endpoints.getAddressesByUser.matchFulfilled,
        (state, action) => {
          state.addresses = action?.payload;
        },
      )
      .addMatcher(
        apiSlice.endpoints.getOrdersByUser.matchFulfilled,
        (state, action) => {
...
export const { setAddress, pushAddress, changeAddress, removeAddress } =
