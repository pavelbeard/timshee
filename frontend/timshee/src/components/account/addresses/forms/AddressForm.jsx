...
const AddressForm = ({ onClose }) => {
  const formContainer = clsx("w-11/12 h-[85%]", "md:w-3/5", "lg:w-2/5");
  const dispatch = useAppDispatch();
  const { t } = useTranslation();
  const address = useSelector(selectCurrentAddress);
  const { currentData: countries, isLoading: isGetCountriesLoading } =
    useGetCountriesQuery();
  const { currentData: provinces, isLoading: isGetProvincesLoading } =
    useGetProvincesQuery();
  const { currentData: phoneCodes, isLoading: isGetPhoneCodesLoading } =
    useGetPhoneCodesQuery();
  ...
  const isLoading =
    isGetCountriesLoading && isGetProvincesLoading && isGetPhoneCodesLoading;
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      phone_code: address.phone_code.country,
    };
      ...
      >
        ...
        >
          <CustomTitle
            title={
              address?.first_name
                ? t("account.forms:editAddress")
                : t("account.forms:addAddress")
            }
          />
          <CustomInput
            htmlFor="first_name"
          ...
          <div className="flex flex-col justify-between lg:flex-row">
            ...
            >
              {[
                ...tmpProvinces.filter(
                  (province) =>
                    province.country.id === address?.province?.country?.id,
                ),
              ]
                .map((item, index) => (
                  <option key={index + 1} value={item?.id}>
          ...
          <CustomCheckbox
            ...
            onChange={(e) =>
              dispatch(setAddress({ ...address, as_primary: e.target.checked }))
            }
          />
