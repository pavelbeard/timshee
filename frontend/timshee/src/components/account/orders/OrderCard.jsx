        ...
        <div className="flex flex-col bg-gray-100 p-6">
            ...
            {order.status === "completed"
                ? (<div className="flex justify-between">
                    ...
                    <span className="roboto-light">{new Date(order?.updated_at).toDateString()}</span>
                </div>)
                : (<>
                        <div className="flex max-sm:flex-col sm:flex-col justify-between">
                            <span className="roboto-light">{t('account.orders:status')}</span>
                            <span className="roboto-light">{t(`account.orders:${toCamelCase(order.status)}`)}</span>
                        </div>
                        <div className="flex max-sm:flex-col sm:flex-col justify-between">
                            <span className="roboto-light">{t('account.orders:createdAt')}</span>
                            <span className="roboto-light">{new Date(order?.created_at).toDateString()}</span>
                        </div>
                    </>
                )
            }
            ...
            <div className="flex justify-between">
                <UnderlinedButton
                    ...
                    className={clsx("roboto-medium hover:text-gray-400 underline underline-offset-4 cursor-pointer")}
                >
                ...
                <UnderlinedButton
                    ...
                    to={!order.non_refundable ? `/orders/${order.second_id}/refund` : undefined}
                >
