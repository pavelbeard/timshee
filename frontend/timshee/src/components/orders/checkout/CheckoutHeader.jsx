    ...
    return(
        <section className="flex flex-col items-center p-6">
            ...
            <div className="flex items-center py-2" data-checkout-stages="">
                <Link className={clsx('m-1', currentStage(1) !== step  && 'text-gray-400')} to={stages[1]}>{title[1]}</Link>
                <ChevronRightIcon strokeWidth="0.5" className="size-4" />
                <Link className={clsx('m-1', currentStage(2) !== step  && 'text-gray-400')} to={stages[2]}>{title[2]}</Link>
                <ChevronRightIcon strokeWidth="0.5" className="size-4" />
                <Link className={clsx('m-1', currentStage(3) !== step  && 'text-gray-400')} to={stages[3]}>{title[3]}</Link>
            </div>
        </section>
    )
}