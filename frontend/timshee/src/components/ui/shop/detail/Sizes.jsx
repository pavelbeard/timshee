...
import {
    selectSizesWithSelection, selectStocks,
...
export default function Sizes() {
    ...
    const initSizes = useCallback(() => {
        ...
        if (size && found) {
            dispatch(setSizesWithSelection([...sizesWithSelection]?.map(s => s.value === size
                ? ({ ...s, selected: true })
                : ({ ...s, selected: false })
            )));
        } else {
    ...
    const inStock = (size) => stocks?.find(s => s.size === size && s.color === get('color'))?.inStock > 0;
    ...
    const setSelection = useCallback((e) => {
        ...
        if (inStock(sizeId)) {
            replace('size', sizeId);
            dispatch(setSizesWithSelection([...sizesWithSelection].map((s, idx) => idx === i
                ? ({...s, selected: true})
                : ({...s, selected: false})
            )));
        }
    ...
    // MEMORIZING ARRAY
    const sizesCircles = useMemo(() => sizesWithSelection.map((size, i) => (
        <li role="option" value={i} id={size?.value} key={i} className={clsx(
            'relative size-8 rounded-2xl border-gray-600 border-[1px] flex justify-center items-center',
            sizesWithSelection.find((s, idx) => idx === i && size?.selected)?.selected &&
            'bg-black text-white',
            inStock(size?.value)
