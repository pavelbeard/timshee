...
import {
    selectColorsWithSelection, selectStocks,
...
export default function Colors() {
    ...
    const initColors = useCallback(() => {
        ...
        if (color && found) {
            dispatch(setColorsWithSelection([...colorsWithSelection].map(c => c.name === color
                ? ({...c, selected: true})
                : ({...c, selected: false})
            )));
        } else {
    ...
    const inStock = (color) => stocks?.find(s => s.size === get('size') && s.color === color)?.inStock > 0;
    ...
    const setSelection = useCallback((e) => {
        ...
        if (inStock(colorId)) {
            ...
            dispatch(setColorsWithSelection([...colorsWithSelection].map((c, idx) => idx === i
                ? ({...c, selected: true})
                : ({...c, selected: false})
            )));
        }
    ...
    // MEMORIZING ARRAY
    const colorsCircles = useMemo(() => colorsWithSelection.map((color, i) => (
        <li role="option" value={i} id={color?.name} key={i} className={clsx(
            'relative size-8 rounded-2xl flex justify-center items-center',
            colorsWithSelection.find((c, idx) => idx === i && c.selected)?.selected &&
            'border-[1px] border-gray-600 opacity-75',
                inStock(color?.name)
        ...
        )} style={{background: color?.hex?.toLowerCase()}} onClick={setSelection}>
        </li>
    )), [setSelection, colorsWithSelection]);

    return (
        <ul role="listbox" className="py-4 grid grid-cols-7 lg:grid-cols-8 items-center justify-items-center justify-center gap-2">
            {colorsCircles}
        </ul>
    );
}