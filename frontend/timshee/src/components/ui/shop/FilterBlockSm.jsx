...
export default function FilterBlockSm({ tag, title, reset, selected, children }) {
    ...
    if (isOpen) {
        return (
            <div className="w-full overflow-y-auto" data-filter-block-sm="">
                <div className="flex bg-white w-full items-center justify-between">
                    <ArrowLeftIcon strokeWidth="0.5" onClick={toggleFilterBlock} className="size-4"/>
                    <div className="flex items-center">
                        <button className="roboto-light text-xs mr-2" onClick={reset}>{t('shop:reset')}</button>
                        <span className="roboto-text">{t('shop:selected')} </span>
                        <span className="roboto-text ml-1">({selected})</span>
                    </div>
                </div>
                <div className="min-h-80 relative overflow-y-auto">
                    {children}
                </div>
            </div>
        )
        ...
        return (
            <div>
                <button className={openBlock ? 'invisible' : 'visible roboto-text'} onClick={toggleFilterBlock}>{title}</button>
