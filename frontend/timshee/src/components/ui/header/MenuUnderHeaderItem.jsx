...
export default function MenuUnderHeaderItem({ apply_gender=null, item=null, withoutList=false }) {
    const id = useId();
    ...
    if (withoutList) {
        ...
        return (
            ...
            >
                <button
                    disabled={disabled}
                    className={clsx("p-2", isOpen && 'bg-gray-100', disabled && 'bg-gray-200 text-gray-500')}
                    aria-expanded="true"
                    aria-haspopup="true"
                    aria-controls={id}
                >
