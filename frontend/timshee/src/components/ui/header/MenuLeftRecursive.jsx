...
import { useAppDispatch, useCategories } from "../../../lib/hooks";
...
export default function MenuLeftRecursive({
  level,
  className,
  openMenus,
  setOpenMenus,
  ...
  const currentCollection = search.get("collections")
    ? `collections=${search.get("collections")}`
    : "";
  ...
  const categoriesList = (url = null) => {
    ...
    const categoriesTmp = categories.map((category, idx) => ({
      ...
      subMenu:
        category?.total !== 0 &&
        typesList(`${url}&categories=${category.code}`, category.types),
      close: true,
    }));
  ...
  return (
    <nav className={clsx(className, "pt-2")}>
