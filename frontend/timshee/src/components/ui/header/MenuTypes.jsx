...
export default function MenuTypes({ id, url, types, categoryImg }) {
    const typesStyle = clsx(
        'p-1 roboto-text-sm',
    );
    const typesData = types?.map((type, index) => type?.total === 0
        ? <span key={index} className={clsx(typesStyle, 'bg-gray-200 text-gray-500 cursor-not-allowed')}>{type.name}</span>
        : <Link
            key={index}
    ...
    return (
        <div id={id} aria-labelledby={id} role='menu' className={clsx(
