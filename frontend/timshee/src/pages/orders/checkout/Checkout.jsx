...
import ToButton from "../../../components/ui/ToButton";
...
import ForShippingAddress
    from "../../../components/orders/checkout/forms/components/checkoutFormHeader/ForShippingAddress";
import ForShippingMethod
    from "../../../components/orders/checkout/forms/components/checkoutFormHeader/ForShippingMethod";

export default function Checkout() {
    window.document.title = 'Checkout | Timshee store'
    const { t } = useTranslation();
    const { width } = useWindowSize();
    const { order, shippingMethods, formData, title, page, formName, handleSubmit, stageName } = useCheckoutFormContext();
    const [showHideCheckoutItems, setShowHideCheckoutItems] = useState(true);
    const shippingPrice = shippingMethods?.find(sm => sm.id === formData?.shipping_method)?.price
        || order?.shipping_price;
    const totalPrice = parseFloat(shippingPrice) + parseFloat(order?.items_total_price);

    return(
        <section className="flex flex-col-reverse justify-end min-h-[100vh] lg:grid lg:grid-cols-2">
            <section className="flex flex-col border-gray-200 lg:border-r-[1px]" data-checkout-container="">
                <CheckoutHeader />
                ...
                <form className="flex flex-col mx-6 mb-24" name={formName[page]} onSubmit={handleSubmit}>
                    ...
                    <div className={clsx('flex', page === 3 ? 'flex-col items-center' : 'xl:flex-row xl:justify-between')}>
                        ...
                        <Button
                            ...
                            type="submit"
                        >
                            {page === 3 ? t('orders.checkout:payment') : stageName[page + 1]?.title}
                        </Button>
                    </div>
                </form>
            </section>
            <section className={clsx('bg-gray-50 flex flex-col p-6')}>
                ...
                <div className={clsx(' h-60 lg:h-80 overflow-y-auto border-b-[1px] border-gray-200',
                    showHideCheckoutItems && width < 1024
                        ? 'hidden'
                        : 'flex flex-col')}>
                ...
                <div className="flex flex-col" data-price-for-all="">
                    <div className="flex items-center justify-between">
                        <span className="tracking-wider lg:text-xl">{t('orders.checkout:subtotal')}</span>
                        <span className="tracking-wider lg:text-xl">{order?.items_total_price}{t('shop:price')}</span>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="tracking-wider lg:text-xl">{t('orders.checkout:shippingPrice')}</span>
                        <span className="tracking-wider lg:text-xl">
                            {shippingPrice}{t('shop:price')}
                        </span>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="tracking-wider text-lg lg:text-2xl">{t('orders.checkout:total')}</span>
                        <span className="tracking-wider text-lg lg:text-2xl">
                            {totalPrice.toFixed(2)}{t('shop:price')}
                        </span>
                    </div>
                </div>
            </section>
