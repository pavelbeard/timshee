...
import {
    useAddWishlistItemMutation, useDeleteWishlistItemMutation,
    useGetItemQuery,
...
const ItemDetail = () => {
    ...
    const cartButtonText = inStock
        ? inCart ? t('shop.itemCardDetail:hasAdded') : t('shop.itemCardDetail:addToCart')
        : t('shop.itemCardDetail:outOfStock')
    const wlButtonText = isThereItemInWL?.length > 0
            ...
            <div className="my-4 flex flex-col lg:grid lg:grid-cols-2 lg:gap-3">
                ...
                <section className="mt-2 lg:mt-0 lg:px-12" data-item-info="">
                    ...
                    <div data-stock-into-cart="">
                        <Button disabled={!inStock || inCart} onClick={addToCart} className="group">
                            {cartButtonText}
                            {inStock &&
                                <ShoppingCartIcon strokeWidth="0.5" className="ml-2 size-4 group-hover:stroke-1" />}
                        </Button>
                    </div>
                    <div className="mt-2" data-stock-add-to-wish="">
                        <button className="underlined-button" onClick={toggleWishlistButton}>
                            {wlButtonText}
                        </button>
                    </div>
                </section>
